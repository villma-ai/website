...
export default function Teaser() {
  ...
  return (
    <>
      <style>{`
        .teaser-pulse-bg {
          animation: teaser-pulse-bg 2s infinite;
        }
        @keyframes teaser-pulse-bg {
          0%, 100% { background-color: rgba(255,255,255,0.80); }
          50% { background-color: rgba(255,255,255,0.30); }
        }
      `}</style>
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 relative overflow-hidden">
        ...
        <div
          ...
          className={`relative z-10 flex flex-col items-center w-full max-w-lg p-6 pb-12 rounded-xl shadow-2xl backdrop-blur-md bg-white transition-all duration-700 ease-in-out border-l-8 border-sky-500${!hovered ? ' teaser-pulse-bg' : ''}`}
          style={{
            transform: visible ? 'translateX(0)' : 'translateX(95%)',
            opacity: visible ? 1 : 0,
            boxShadow: visible ? '0 0 40px 0 rgba(0,0,0,0.15)' : 'none',
          }}
        >
